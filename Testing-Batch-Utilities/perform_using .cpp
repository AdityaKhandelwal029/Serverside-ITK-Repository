...
int ITK_user_main(int argc, char* argv[])
{
	...
	if (x == 1)
	{
		...
		relationtype.push_back("IMAN_Rendering");
		...
		char *error_text;
		try {
			error_code = ITEM_perform_deepcopy_using2(newrevt, op, lstitemrevt, 1, secondaryobjlist, p_attr_copyoptions1.get(), p_attr_optionname1.get(), p_attr_relationtype1.get(), p_attr_relationtype1.get(), &countofobjects, &deepcopiedobjects);
