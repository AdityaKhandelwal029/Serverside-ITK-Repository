...
int ITK_user_main(int argc, char* argv[])
{
	...
	if (x == 1)
	{
		char *d_value,*d_value1;
		tag_t item_t;
		long long int x; 

		cout<<" Item-found-status - "<<ITEM_find_item("1409280", &item_t)<<endl; 
		cout << AOM_UIF_ask_value(item_t, "object_string", &d_value1) << endl;
		cout << " Name = " << d_value1 << endl;
		...
		if (x != 0)
		{
			...
			EMH_ask_error_text(x, &d_value); 
			cout << " Reason of the error = " << d_value << endl;
			int no_of_reference;
			tag_t *ref_instances; 
			int *instance_levels;
			...
			for (int i = 0; i < no_of_reference; i++)
			{
				...
				AOM_UIF_ask_value(relation_t, "object_name", &relation_name); 
