...
int ITK_user_main(int argc, char* argv[])
{
	...
	if (x == 1)
	{
		...
		cout << " op = " << op << endl;
		try {
			cout << ITEM_perform_deepcopy_using(newrevt, op, lstitemrevt, 1, secondaryobjlist, p_attr_copyoptions1.get(), p_attr_optionname1.get(), p_attr_relationtype1.get(), &countofobjects, &deepcopiedobjects);
