...
void BOM_ask_child(int c, tag_t *k)
{
	...
	for (int i = 0; i < c; i++)
	{
		...
		for (p = m2.begin(); p != m2.end(); p++)
		{
			string q1 = *p;
			char *d_value;
			const char *ge_value = q1.c_str();
			AOM_UIF_ask_value(k[i], ge_value, &d_value);
			cout << " q1 = " << q1;
			cout << " ge_value = " << d_value << endl; 
			q1 = d_value;
	...
	if (x == 1)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{ 
				...
				cout << " Item_found_successfully"<<endl;
				fx1();
				if (count > 0)
				{
		...
		if (found1 == 0)
		{
			cout << " Revision not found or any error occurred " << endl;
		}
	}
	return(0);
}