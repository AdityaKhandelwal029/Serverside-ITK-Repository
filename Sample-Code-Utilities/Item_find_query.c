...
int ITK_user_main(int argc, char*argv[])
{
	...
	cEntry_Names[0] = "Name";
	cEntry_Names[1] = "Item ID";
	cValues[0] = "Check_04";
	cValues[1] = "000074";
	Error_check(QRY_find2("Item...", &tQuery)); //API to find created query
	Error_check(QRY_execute(tQuery, 2, cEntry_Names, cValues, &No_of_obj, &tObject));//API to run query
	for (i = 0; i < No_of_obj; i++)
	{
		Error_check(AOM_UIF_ask_value(tObject[i], "object_name", &Obj_name));//API to get property name
		printf("\nObject Name is %s", Obj_name);
		Error_check(AOM_UIF_ask_value(tObject[i], "object_type", &Obj_type));//to get property name
		printf("\nObject type is %s", Obj_type);
	}
	if (tQuery)
	{
		MEM_free(tObject);
