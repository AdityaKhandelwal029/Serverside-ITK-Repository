/*Objective - To convert date .
   Api's used - DATE_string_to_date()
...
int ITK_user_main (int argc, char ** argv)
{
	...
	FILE *flogPtr=fopen("/user/testcmi/Dev/devgroups/Aditya/testfolder/Datetest2.txt","w+");
	        ...
	        if(itkApiCallStatus==ITK_ok)
			    {
					...
					ITK_APICALL(ITEM_find_item("008424",&itemTag),flogPtr);
					fprintf(flogPtr,"\n Item with given id-008424 found. ");
					...
					ITK_APICALL(DATE_string_to_date(creationDate,dateFormatStr,&month,&day,&year,&hour,&minute,&second),flogPtr);	
                    month=month+1;					
                    fprintf(flogPtr,"\n -month       =  : %d",month);
					fprintf(flogPtr,"\n -day         =  : %d",day);
					fprintf(flogPtr,"\n -year        =  : %d",year);
