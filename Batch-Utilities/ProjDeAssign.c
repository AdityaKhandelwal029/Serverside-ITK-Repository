	...
	for(i=0; i<totalelmt; i++)
	{
		...
		if(number_found>0)
		{
			...
			//PROJ_add_author_members(project_tag,1, user_tag_objs);
			 ...
			 iStatus1 = PROJ_remove_members(project_tag,1, user_tag_objs);
			
			//iStatus1 = PROJ_remove_author_members(project_tag,1, user_tag_objs);
			
			printf("\nPRIVILEDGE Removed from USER  = %d\n",iStatus1); fflush(stdout);
		}
		else
		{
